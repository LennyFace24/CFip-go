...
// 内容按「达标优先、延迟升序、失败最后」排列，便于直接阅读与比对。
func WriteSpeedLog(rows []SpeedResult, summary SpeedSummary, cfg *config.Config) (string, error) {
	...
	sort.SliceStable(sorted, func(i, j int) bool {
		qi, qj := isQualified(sorted[i], cfg.Latency), isQualified(sorted[j], cfg.Latency)
		if qi != qj {
			return qi
		}
	...
	fmt.Fprintf(&b, "探测: %d    达标: %d    超标: %d    失败: %d    耗时: %d ms    %s\n",
		summary.Total, summary.Qualified, summary.OverLimit, summary.Failed, summary.Elapsed,
		map[bool]string{true: "提前结束", false: "全部跑完"}[summary.Stopped])
	b.WriteString("\n")

	w := tabwriter.NewWriter(&b, 0, 0, 3, ' ', 0)
	fmt.Fprintln(w, "IP\t延迟(ms)\t状态\t来源")
